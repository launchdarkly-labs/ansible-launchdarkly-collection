...
def main():
    ...
    if feature_flags.get("items"):
        ...
        flags = feature_flags
    module.exit_json(changed=True, results=feature_flags)
        ...
        if module.params.get("key"):
            if module.params.get("env"):
                response = api_instance.get_feature_flag(
                    module.params["project_key"], module.params["key"], env=module.params["env"]
                )
            else:
                response = api_instance.get_feature_flag(
            ...
            filtered_keys = dict((k, module.params[k]) for k in keys if k in module.params and module.params[k] is not None)
            response = api_instance.get_feature_flags(
                **filtered_keys
            )
